...
class OrderRequest(BaseModel):
    ...
    # Idempotency key (the venue's client order id / cOID). Generated by the safety layer
    # BEFORE dispatch and threaded down so the venue's order carries the same id — this is
    # what lets a dispatched-but-unconfirmed order (timeout/crash) be reconciled against the
    # venue's open orders and blocks a duplicate live order on retry. Not an order parameter.
    client_order_id: str | None = Field(default=None, description="Idempotency key (cOID).")
