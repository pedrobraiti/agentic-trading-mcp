...
## 2026-06-22 — `cashQty` é SÓ para compra; venda fracionária é por quantidade de ações
**Motivo:** validação ao vivo (mercado aberto, conta real `U24235856`) de um round-trip US$2 em AAPL revelou que a IBKR **rejeita `cashQty` em ordens de venda**: `"Cash order quantity can not be set for sell order"` (rejeição de sistema, ordem 864501613). Ou seja, o fracionário por valor em US$ funciona só no BUY. Para **fechar/reduzir** uma posição fracionária é obrigatório enviar `quantity` **fracionária** (ex.: 0.0066 ações) — que foi como a venda finalmente executou (ordem 864501623, Filled @ 300.41, caixa recuperado).
**Implicação no código:** `OrderRequest.quantity` é `int` hoje → **não consegue fechar posição fracionária**. Correção necessária: aceitar `quantity` fracionária (`Decimal`) no model/broker/tools, e idealmente uma primitiva `close_position` que lê o tamanho exato da posição e vende tudo (evita o warning de oversell e o cashQty).
**Decisão sobre `o2137`:** o warning de venda `o2137` ("closing order quantity is greater than your current position") **NÃO** entra na allow-list global — auto-confirmar "vender mais do que se tem" tem semântica perigosa (oversell/short) e é exatamente o tipo de coisa que a trava deve barrar. O caminho correto é vender a quantidade exata, sem disparar o warning.
**MessageIds de reply mapeados ao vivo (allow-list de BUY MKT+cashQty):** `o354`, `o10164` (Market Order Confirmation), `o10223` (Mandatory Cap Price), `o10151` (cash quantity disclaimer), `o10153` (Cash Quantity Order Confirmation). Todos `isSuppressible:true`/"Accept and Continue".
